...
CREATE TABLE IF NOT EXISTS `purchase_orders` (
    ...
    `type` ENUM('Purchasing Order', 'Backorder') DEFAULT 'Purchasing Order',
    ...
    `supplier_id` BIGINT UNSIGNED DEFAULT 3,
    FOREIGN KEY (`supplier_id`) REFERENCES `user`(`user_id`) ON DELETE CASCADE,
    
    -- Foreign Key for order_status
    `order_status` SMALLINT UNSIGNED DEFAULT 1,
    FOREIGN KEY (`order_status`) REFERENCES `order_statuses`(`ostatus_id`) ON DELETE CASCADE,
